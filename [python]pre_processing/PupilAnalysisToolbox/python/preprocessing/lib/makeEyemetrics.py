        ...
        while ( currentTime < (len(indx)-1) ):
            if indx[currentTime+1]-indx[currentTime]==1:
                ...
                if (dur>=MINDUR) & (d >= 20):
                    ...
                    x1 = np.mean(np.array(tx)[indx[sTime]-3])
                    x2 = np.mean(np.array(tx)[indx[sTime]])
                    
                    y1 = np.mean(np.array(ty)[indx[sTime]-3])
                    y2 = np.mean(np.array(ty)[indx[sTime]])
        ...
        for ev in events[:1]:
           for e in ev:
               ...
               if len(e["gaze_x"])>0:
                   ...
                   x1 = np.mean(e["gaze_x"][47])
                   x2 = np.mean(e["gaze_x"][50])
                
                   y1 = np.mean(e["gaze_y"][47])
                   y2 = np.mean(e["gaze_y"][50])
        
                   plt.plot(e["gaze_x"],e["gaze_y"],'.-')
                   # plt.plot(e["gaze_x"][47:53],e["gaze_y"][47:53],'r.-')
                   plt.plot(np.array([x1,x2]),np.array([y1,y2]),'ko-')
                   plt.plot(x1,y1,'y^')
